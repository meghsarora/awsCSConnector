...
@Component(service= Servlet.class,
		...
		property={
				...
				"sling.servlet.paths=/bin/aws/awsaveanalysisschemeservlet"

		})
//@SlingServlet(methods = { "POST" }, paths = { "/bin/aws/schemeservlet" })
public class AWSSaveAnalysisSchemeServlet extends SlingAllMethodsServlet {
	...
	private static final Logger LOG = LoggerFactory.getLogger(AWSSaveAnalysisSchemeServlet.class);
		...
		try {
			...
			synonym.append("}");
				...
				LOG.info("******** language after change: "+language);
					...
					JsonObject jsonObject = indexService.createAnalysisScheme(comp.trim(), synonym.toString(),stopword.toString(),language);
